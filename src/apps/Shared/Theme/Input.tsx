...
const CustomInput = forwardRef<HTMLInputElement, CustomInputProps>((
  ...
  return (
    <div className='flex flex-col gap-y-2'>
      {label && (
        <label
          className={`ml-1 text-sm leading-5 font-raleway font-medium ${focus ? 'text-primary-400' : 'text-primary-500'}`}
          htmlFor={args.id}>
          {label}
        </label>
      )}
      <TextField 
        ...
        InputProps={{
          ...
          ...(args.disabled && clipboard && args.type !== 'password' && {
            endAdornment: <span 
              className='z-20 p-1 rounded-md border border-primary-400 text-primary-400 hover:cursor-pointer'
        ...
        inputProps={{
          ...
          className: `rounded-[inherit] py-3 px-3 text-primary-600 font-medium ${args.inputProps?.className ?? new String()}`
        }}
        ref={(nativeElement) => {
